...
export const FormButton = ({ text, onClick, type = "submit", isLoading = false }) => {
  return (
    <button 
      type={type} 
      onClick={onClick}
      disabled={isLoading}
      className={`button-glow w-full text-white font-medium py-3 px-4 rounded-full transition duration-200 ${isLoading ? 'cursor-not-allowed opacity-20' : 'cursor-pointer'} mt-2`}
    >
      {isLoading ? (

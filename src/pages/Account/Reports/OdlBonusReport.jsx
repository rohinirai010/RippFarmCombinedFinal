...
} from "../../../partials/account/ReportsReusableComponents";

// Odl Bonus Report Page Component
const OdlBonusReport = () => {
  ...
  const [allData, setAllData] = useState([
    {
      id: 1,
      date: "2025-04-11 00:30:02",
      particulars: "Daily Bot Profit",
      amount: 6.75,
      status: "Active",
    },
    {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      render: (value) => (
        <span className="font-medium text-green-400">{value.toFixed(2)}</span>
      ),
    },
    {
      key: "status",
  ...
  const calculateSummaryData = (filteredData) => {
    ...
    const averageDaily =
      uniqueDates.length > 0 ? totalProfit / uniqueDates.length : 0;

    // Calculate percent change from previous month (mock calculation)
    // In a real app,  compare with actual previous month data
  ...
  const handleExport = (format, dataToExport) => {
    ...
    exportDataUtil(formattedData, format, "odl-bonus-report");
  };
    ...
    >
      ...
      <motion.header
        className={`sticky top-0 bg-[#1d1e35] z-30 transition-all duration-300 ${
          isScrolled
            ? "bg-opacity-95 backdrop-blur-md shadow-lg shadow-blue-900/10"
      ...
      <div className="flex flex-row justify-between mt-2 pr-4">
        ...
        <div className="px-4 py-2 flex flex-wrap items-center gap-2 text-xs text-gray-400">
          ...
          {!startDate && !endDate && !searchQuery && <span>None applied</span>}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          >
            ...
            <div className="text-base sm:text-xl font-bold text-green-400">
              {summaryData.totalProfit.toFixed(2)} USDT
            </div>
          ...
          >
            ...
            <div className="text-base sm:text-xl font-bold text-blue-400">
              {summaryData.averageDaily.toFixed(2)} USDT
            </div>
            <div className="text-[11px] sm:text-xs text-blue-300 mt-1">
